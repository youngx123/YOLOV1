...
class Loss(nn.Module):
    ...
    def __init__(self, num_bboxes=2, num_classes=20, lambda_coord=5.0, lambda_noobj=0.5):
        """ Constructor.
        Args:
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        for i in range(0, bbox_target.size(0), B):
            ...
            # Because (center_x,center_y)=pred[:, 2] and (w,h)=pred[:,2:4] are normalized
            # for cell-size and image-size respectively, rescale (center_x,center_y) for
            ...
            # Because (center_x,center_y)=target[:, 2] and (w,h)=target[:,2:4] are normalized
            # for cell-size and image-size respectively, rescale (center_x,center_y) for the
            # image-size to compute IoU correctly.
            ...
            # "we want the confidence score to equal the (IOU) between the predicted box and the ground truth"
            # from the original paper of YOLO.
            bbox_target_iou[i + max_index, torch.LongTensor([4]).cuda()] = 1
        ...
        loss_wh = F.mse_loss(torch.sqrt(bbox_pred_response[:, 2:4]), torch.sqrt(bbox_target_response[:, 2:4]),
                             reduction='sum')
        loss_obj = F.binary_cross_entropy(bbox_pred_response[:, 4], target_iou[:, 4], reduction='sum')
