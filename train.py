...
def parse_args():
    ...
    parser.add_argument('--pretrained_model', default="./weights/best.pth", type=str, help='load pretrained model')

    return parser.parse_args()


def save_checkpoint(state_dict, optimizer, config):
    """
    ...
    config : config files
    """
    checkpoint_path = os.path.join(config.save_folder, f"model.pth")
    model_dict = {
...
def train_model(config, net, loss_fn, optimizer, trainloader, valloader):
    ...
    for epoch in range(EPOCH):
        ...
        for step, batch in enumerate(progressBar):
            ...
            if ni <= n_burn:
                for j, x in enumerate(optimizer.param_groups):
                    ...
                    x['lr'] = np.interp(ni, [0, n_burn], [0.1 if j == 2 else 0.0, 0.001* lf(epoch)])
                    if 'momentum' in x:
        ...
        if epoch % config.eval_epoch == 0:
            save_checkpoint(net.state_dict(), optimizer, config)
            val_loss = eval_model(net, loss_fn, valloader)
            if best_val > val_loss:
                logging.info("validate loss improve from  %.4f to  %.4f " % (best_val, val_loss))
                best_val = val_loss
                checkpoint_path = os.path.join(config.save_folder, "best.pth")
                model_dict = {
                    "model_weight": net.state_dict(),
                    "optimizer": optimizer.state_dict(),
                }
                torch.save(model_dict, checkpoint_path)
                logging.info("save best  checkpoint saved to %s" % checkpoint_path)
            else:
...
def main(config):
    ...
    traindataset = yoloDataset(train_Text, imageSie=train_size[0], classNum=classNum)
    val_dataset = yoloDataset(val_Text, imageSie=train_size[0], classNum=classNum)
    ...
    #         params += [{'params': [value], 'lr': 0.001 * 1}]
    #     else:
